...
-- MISC CONFIG
local commsport = 125
local screensaveractivate = 20
local modem = "local"                -- Set to nil to use built in modem or tunnel card, "local" to use local events only
-- END CONFIG
...
local noscreensaver = false
local qtyselector = nil
local delgado = nil

if next(c.list("tablet")) then
--   print("This is a tablet, disabled screensaver")
   noscreensaver = true
end
...
local function comms(c1, c2, c3, c4, c5, c6) 
   if modem == "local" then
      if c1 == "itemlist" then
         computer.pushSignal(c1, "local", c2, c3, c4, c5, c6)
      else
         print("Sending ", c1, c2, c3, c4, c5, c6)
         computer.pushSignal(c1, c2, c3, c4, c5, c6)
      end
   else
